  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 py-10">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-gray-900">支付定金</h1>
          <p className="text-sm text-gray-500 mt-1">
            确认行程并支付 10% 定金，锁定您的专属司机
          </p>
        </div>

        {/* Order summary */}
        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-5">
          <h2 className="font-semibold text-gray-900 mb-4">行程摘要</h2>
          <div className="space-y-3 text-sm">
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              <div>
                <p className="text-xs text-gray-400 mb-0.5">出行日期</p>
                <p className="text-gray-800">{formatDate(order.demand.travel_date)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-0.5">乘客人数</p>
                <p className="text-gray-800">{order.demand.pax_count} 人</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-0.5">司机</p>
                <p className="text-gray-800">{(order.bid as any).driver?.full_name ?? '司机'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-0.5">车型</p>
        ...
        <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-6 text-sm text-blue-800">
          ...
          <span>
            定金由平台托管，行程完成后自动结算给司机。行程取消后可申请退款。
          </span>
        </div>
